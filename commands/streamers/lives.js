    ...
    try {
      ...
      if (!streamer) {
        await interaction.editReply({
          content: '<:trupe_aviso:1535106703870271489> Você não está registrado como streamer oficial. Peça a um ADM para usar `/addstreamer`.',
        });
        return;
      }
      ...
      if (!CANAIS.lives) {
        await interaction.editReply({
          content: '<:trupe_aviso:1535106703870271489> O canal de lives ainda não foi configurado. Peça a um ADM para configurar em `/config`.',
        });
        return;
      }
      ...
      if (!canal || !canal.isTextBased()) {
        await interaction.editReply({
          content: '<:trupe_aviso:1535106703870271489> O canal de lives configurado não foi encontrado. Peça a um ADM para reconfigurar em `/config`.',
        });
        return;
      }
      ...
      const container = buildContainer({
        ...
        corpo: `**<@${interaction.user.id}>** está ao vivo na Twitch! Vem ver! <:trupe_live:1535106655681912955>\n\n<:trupe_twitch:1535106681124556942> **Canal:** ${streamer.get('Canal Twitch')}`,
      ...
      await interaction.editReply({
        content: `<:trupe_sucesso:1535106719305175122> Aviso de live enviado em <#${canal.id}>!`,
      });
    } catch (error) {
      console.error('Erro no /lives:', error);
      try {
        const payload = {
          content: '<:trupe_erro:1535106712359407626> Ocorreu um erro ao enviar o aviso de live. Tente novamente.',
          ephemeral: true,
        };
