    ...
    try {
      ...
      if (!streamer) {
        await interaction.editReply({
          content: '<:trupe_aviso:1535757212541128724> Você não está registrado como streamer oficial. Peça a um ADM para usar `/addstreamer`.',
        });
        return;
      }
      ...
      if (!CANAIS.lives) {
        await interaction.editReply({
          content: '<:trupe_aviso:1535757212541128724> O canal de lives ainda não foi configurado. Peça a um ADM para configurar em `/config`.',
        });
        return;
      }
      ...
      if (!canal || !canal.isTextBased()) {
        await interaction.editReply({
          content: '<:trupe_aviso:1535757212541128724> O canal de lives configurado não foi encontrado. Peça a um ADM para reconfigurar em `/config`.',
        });
        return;
      }
      ...
      await interaction.editReply({
        content: `<:trupe_sucesso:1535757248930775041> Aviso de live enviado em <#${canal.id}>!`,
      });
    } catch (error) {
      console.error('Erro no /lives:', error);
      try {
        const payload = {
          content: '<:trupe_erro:1535757225631686686> Ocorreu um erro ao enviar o aviso de live. Tente novamente.',
          ephemeral: true,
        };
