  ...
  async execute(interaction) {
    try {
      ...
      await interaction.editReply({
        content: `<:trupe_sucesso:1535106719305175122> **${mensagensDeletadas.size}** mensagem(ns) deletada(s) com sucesso.`,
      });
    } catch (error) {
      console.error('Erro no /clear:', error);
      try {
        const payload = {
          content: '<:trupe_erro:1535106712359407626> Ocorreu um erro ao deletar as mensagens. Mensagens com mais de 14 dias não podem ser deletadas em massa.',
          ephemeral: true,
        };
